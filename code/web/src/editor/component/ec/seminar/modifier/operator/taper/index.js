 ...
 * Taper 锥化
 */
export default class Taper extends Base {
    /**
     * 设置面板
     */
    #setter;
    ...
    start() {
        ...
        this.setting_panel_container.setHeaderDefault('modifier.taper');
    }
    ...
    #onSetterChanged() {
        ...
        const {
            CoordinateDir,
            GeoSolidSoupOP,
            GeoSolidSoupOP_TaperContext,
        } = Chameleon;

        // 如果 Dir 变化了
        const dir = this.#setter.coordinateDir;
        const percent = this.#setter.percent;
        if (this.#dir !== dir) {
            this.#dir = dir;
            ...
            this.#op_context = new GeoSolidSoupOP_TaperContext(this.#soup_current);

            // 处理
            if      ('px' === dir) this.#op.taper(percent, CoordinateDir.PX, this.#op_context);
            else if ('py' === dir) this.#op.taper(percent, CoordinateDir.PY, this.#op_context);
            else if ('pz' === dir) this.#op.taper(percent, CoordinateDir.PZ, this.#op_context);
            else if ('nx' === dir) this.#op.taper(percent, CoordinateDir.NX, this.#op_context);
            else if ('ny' === dir) this.#op.taper(percent, CoordinateDir.NY, this.#op_context);
            else if ('nz' === dir) this.#op.taper(percent, CoordinateDir.NZ, this.#op_context);
            ...
            // 执行修改
            this.#op_context.setPercentOffset(percent);
