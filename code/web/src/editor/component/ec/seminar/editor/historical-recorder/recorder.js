...
export default class Recorder extends HistoricalRecorder {
    ...
    constructor(ec, coordinator, connector) {
        ...
        this.#connector_historic_recorder = connector.getHistoricRecorder();
    }
    ...
    rollback() {
        ...
        if (!historic_recorder.canRollback()) {
            return false;
        }

        if (historic_recorder.isCurrentPlaceholder()) {
    ...
    append(item) {
        ...
        while (historic_recorder.size() >= this.#max_recorder_size) {
            if (historic_recorder.isFrontPlaceholder()) {
                ...
                if (front && isFunction(front.destroy)) {
                    front.destroy();
                }
            }
