...
export default class Cylinder extends Base {
    ...
    start() {
        ...
        this.setting_panel_container.setHeaderDefault('shape.cylinder');
        this.deferDestory(() => this.#setter.dispose());

        // 构建 Mesh
        this.#mesh = new EditableMesh();
        this.#mesh.setName(`Cylinder - ${this.unique_id}`);
    ...
    #updateGeo(n_subdivisions, radius, h) {
        ...
        GeoSolidSoupEmbeddedShapes.Cylinder(
            soup.getPtr(), 
            n_subdivisions, 
            radius, 
            h);
